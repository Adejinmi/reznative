...
export default SingleQuestion = ({ route, navigation}) =>{
    const {ID, Question, Options, CorrectOption } = route.params
    const { className } = useContext (AppContext)
    const [question, setQuestion] = useState(Question)
    const [options, setOptions] = useState(JSON.parse(Options))
    const [correctOpt, setCorrectopt] = useState(CorrectOption)
    ...
    const saveQuestion = async()=>{
      await db.transaction(async(tx)=>{
        ...
        tx.executeSql(`UPDATE ${query} SET Question = '${question}', Options = '${opt}', CorrectOption='${correctOpt}' WHERE ID = ${ID}`,[],
        (tx, results)=>{
    ...
    return(
        ...
        <SafeAreaView style={styles.mainone}>
         ...
         <View style={[styles.section, styles.sectionElevation, {minHeight:100, padding:10}]}>
            ...
            <TextInput value={correctOpt} style={{fontFamily:"futura_medium", color:"green", fontWeight:"bold", padding:0}} editable={editing} onChangeText={(value)=>{setCorrectopt(value)}} />
